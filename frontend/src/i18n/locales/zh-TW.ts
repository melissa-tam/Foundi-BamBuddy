export default {
  ...
  productionRuns: {
    ...
    fields: {
      ...
      noEjectProfile: '無',
      skuDefault: 'SKU 預設',
      cooldownOverride: '覆寫冷卻溫度 (°C)',
      cooldownPlaceholder: '使用設定檔預設值',
      cooldownProfileDefault: '覆寫設定檔預設值 {{value}}°C',
      skuNoFile: '— 未連結檔案',
  ...
  ejectProfiles: {
    ...
    tooltips: {
      ...
      cooldownTemp: '列印件在開始清掃前必須冷卻到的熱床溫度 – 冷卻後零件會從列印板脫離。',
    ...
    geometry: {
      ...
      cooldownAmbientTrap: '低於冷卻警告下限（{{floor}} °C）。等於或低於工作間環境溫度的閾值永遠無法達到，取件等待將永久卡住 — 請設定為高於環境溫度幾度。',
    ...
    queueHelp: '當熱床冷卻至低於設定檔閾值後，零件將被掃下。',
  },
  // Navigation
  nav: {
    printers: '印表機',
  ...
  settings: {
    ...
    farmCooldownWarnFloor: '冷卻警告下限（°C）',
    farmCooldownWarnFloorHelp: '當取件設定檔的冷卻閾值低於此值時發出警告 — 等於或低於工作間環境溫度的閾值永遠不會完成（15–50）',
    ...
    farmCooldownPlateauMarginHelp: '若降溫停滯但熱床處於取件溫度的該溫差範圍內，則視為已冷卻並取件；若卡在更高溫度，則將印表機隔離。',
