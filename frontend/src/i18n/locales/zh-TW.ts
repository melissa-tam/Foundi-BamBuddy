export default {
  ...
  skus: {
    ...
    suggest: {
      title: '從檔案建議',
      description: '選擇一個已切片的檔案，從其中繼資料預先填入編碼、零件編號和名稱。',
      file: '庫檔案',
      filePlaceholder: '選擇一個檔案…',
      action: '建議',
      applied: '已從 {{source}} 填入資訊',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: '為沒有 RFID 標籤的 AMS 料盤自動建立庫存料卷。如果你手動預先登記料卷以避免重複，請關閉此項。',
    ...
    farmCooldownStallWindow: '冷卻停滯視窗（分鐘）',
    farmCooldownStallWindowHelp: '取件冷卻期間熱床必須持續降溫；連續兩個視窗沒有進展則將印表機隔離（0 為停用）',
    farmCooldownStallEpsilon: '每視窗最小降溫（°C）',
    farmCooldownStallEpsilonHelp: '每個視窗算作進展的最小降溫量（°C）',
    farmCooldownMaxHold: '最長冷卻等待（分鐘）',
    farmCooldownMaxHoldHelp: '在高於釋放閾值超過這麼多分鐘後，仍會執行取件（0 = 無限期等待閾值）',
    farmCooldownPlateauMargin: '平台期頂出餘量 (°C)',
    farmCooldownPlateauMarginHelp: '若冷却在釋放閾值的該溫度範圍內停滞，則頂出（熱床已降至環境溫度）而非隔離印表機。超過閾值＋餘量則熱床確實卡在高溫，印表機將被隔離',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: '目前記錄 #{{id}} — 剩餘 {{grams}} g — 將被封存',
