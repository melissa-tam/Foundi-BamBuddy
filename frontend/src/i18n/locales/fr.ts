export default {
  ...
  skus: {
    ...
    suggest: {
      title: 'Suggérer depuis un fichier',
      description: 'Choisissez un fichier tranché pour préremplir le code, le numéro de pièce et le nom à partir de ses métadonnées.',
      file: 'Fichier de bibliothèque',
      filePlaceholder: 'Sélectionnez un fichier…',
      action: 'Suggérer',
      applied: 'Données remplies depuis {{source}}',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: "Créer silencieusement une bobine d'inventaire pour les bacs AMS sans étiquette RFID. Désactivez si vous préenregistrez les bobines manuellement pour éviter les doublons.",
    ...
    farmCooldownStallWindow: 'Fenêtre de blocage du refroidissement (min)',
    farmCooldownStallWindowHelp: 'Pendant le refroidissement d\'éjection, le plateau doit continuer à refroidir ; deux fenêtres consécutives sans progression mettent l\'imprimante en quarantaine (0 désactive)',
    farmCooldownStallEpsilon: 'Refroidissement min. par fenêtre (°C)',
    farmCooldownStallEpsilonHelp: 'Refroidissement minimal (°C) par fenêtre comptant comme une progression',
    farmCooldownMaxHold: 'Attente max. de refroidissement (min)',
    farmCooldownMaxHoldHelp: 'Après ce nombre de minutes au-dessus du seuil de libération, l\'éjection s\'exécute quand même (0 = attendre le seuil indéfiniment)',
    farmCooldownPlateauMargin: 'Marge d’éjection sur plateau (°C)',
    farmCooldownPlateauMarginHelp: 'Si le refroidissement stagne à moins de ce nombre de °C du seuil de libération, éjecter (le plateau est à température ambiante) au lieu de mettre en quarantaine. Au-dessus du seuil + marge, le plateau est réellement bloqué à chaud et l’imprimante est mise en quarantaine',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: 'Fiche actuelle n° {{id}} — {{grams}} g restants — sera archivée',
