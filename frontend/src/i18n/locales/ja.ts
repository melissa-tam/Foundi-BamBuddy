export default {
  ...
  ejectProfiles: {
    ...
    tooltips: {
      ...
      cooldownRetries: '目標に達するのを待つ間、ベッド温度を再確認する回数（これを超えると中止します）。',
  ...
  printers: {
    ...
    phase: {
      printing: '印刷中',
      cooling: '{{threshold}}°C まで冷却中（ベッド {{bed}}°C）— 排出スイープはファイル内で実行されます',
  ...
  settings: {
    ...
    farmRetryMaxPerUnitHelp: 'プレートが失敗と判定されるまでの自動再試行回数（0〜5）',
    farmEscalateConsecutiveFailures: '失敗後の隔離',
    farmEscalateConsecutiveFailuresHelp: '隔離を発動するプリンターの連続失敗回数（1〜10）',
