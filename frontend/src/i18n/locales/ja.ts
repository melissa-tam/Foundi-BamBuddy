export default {
  ...
  skus: {
    ...
    suggest: {
      title: 'ファイルから提案',
      description: 'スライス済みファイルを選ぶと、そのメタデータからコード・部品番号・名前を自動入力します。',
      file: 'ライブラリファイル',
      filePlaceholder: 'ファイルを選択…',
      action: '提案',
      applied: '{{source}} から情報を入力しました',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: 'RFID タグのない AMS トレイに対して在庫スプールを自動作成します。重複を避けるためにスプールを手動で事前登録する場合はオフにしてください。',
    ...
    farmCooldownStallWindow: '冷却停滞ウィンドウ（分）',
    farmCooldownStallWindowHelp: '排出の冷却中、ベッドは冷え続ける必要があります。進捗のないウィンドウが2回連続するとプリンターを隔離します（0で無効）',
    farmCooldownStallEpsilon: 'ウィンドウあたり最小冷却（°C）',
    farmCooldownStallEpsilonHelp: '進捗とみなすウィンドウあたりの最小冷却量（°C）',
    farmCooldownMaxHold: '最大冷却待機（分）',
    farmCooldownMaxHoldHelp: 'リリースしきい値を超えてこの分数が経過すると、排出を強制実行します（0 = しきい値まで無期限に待機）',
    farmCooldownPlateauMargin: 'プラトー排出マージン (°C)',
    farmCooldownPlateauMarginHelp: '冷却が解放しきい値からこの°C以内で停滞した場合、隔離せずに排出します（ベッドは室温で安定）。しきい値＋マージンを超える場合、ベッドは本当に高温のままで、プリンターは隔離されます',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: '現在の記録 #{{id}} — 残り {{grams}} g — アーカイブされます',
