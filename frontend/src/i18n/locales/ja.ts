export default {
  ...
  settings: {
    ...
    ejectUploadSkipIdentical: 'USBドライブに既にある取り出しファイルを再利用',
    ejectUploadSkipIdenticalHelp: '同一の取り出しファイルが既にプリンターのUSBドライブにある場合は再アップロードを省き、取り出しごとに数秒節約します。',
    ejectUploadSkipIdenticalWarning: 'ライブ取り出しプローブでドライブ上のファイルが正しく再利用されると確認できてからのみ有効にしてください。不安な場合はオフのままに。再アップロードすれば常に正しいファイルが保証されます。',
    ejectSlim3mf: '取り出しファイルを軽量化',
    ejectSlim3mfHelp: '取り出しファイルから3Dモデルとプレビュー画像を取り除いてアップロードを高速化します。取り出し動作自体は変わりません。',
    ejectSlim3mfWarning: 'ハードウェア制限あり：無人生産で有効にする前に、軽量化ファイルで取り出しハードウェアラダーを完走してください。そのラダーに合格するまではオフのままに。',
    powerStagger: '電力スタガー',
