export default {
  ...
  productionRuns: {
    ...
    fields: {
      ...
      cooldownOverride: '冷却の上書き (°C)',
      cooldownPlaceholder: 'プロファイルのデフォルトを使用',
      cooldownProfileDefault: 'プロファイルのデフォルト {{value}}°C を上書きします',
      skuNoFile: '— ファイル未リンク',
  ...
  ejectProfiles: {
    ...
    tooltips: {
      ...
      cooldownTemp: 'スイープを始める前にプリントが冷える必要があるベッド温度。冷えると部品がプレートから外れます。',
    ...
    geometry: {
      ...
      cooldownAmbientTrap: '冷却警告下限（{{floor}} °C）を下回っています。工場の室温以下のしきい値には決して到達できず、取り出し待機が永久に停止します — 室温より数度高く設定してください。',
    ...
    queueHelp: 'ベッドがプロファイルのしきい値まで冷えると、部品が払い出されます。',
  },
  // Navigation
  nav: {
    printers: 'プリンター',
  ...
  settings: {
    ...
    farmCooldownWarnFloor: '冷却警告下限（°C）',
    farmCooldownWarnFloorHelp: '取り出しプロファイルの冷却しきい値がこれを下回ると警告 — 室温以下のしきい値は決して完了しません（15〜50）',
