export default {
  ...
  skus: {
    ...
    suggest: {
      title: 'Dosyadan öner',
      description: 'Kod, parça numarası ve adı meta verilerinden önceden doldurmak için dilimlenmiş bir dosya seçin.',
      file: 'Kitaplık dosyası',
      filePlaceholder: 'Bir dosya seçin…',
      action: 'Öner',
      applied: 'Bilgiler {{source}} kaynağından dolduruldu',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: 'RFID etiketi olmayan AMS tepsileri için otomatik olarak bir envanter makarası oluşturur. Yinelenenleri önlemek için makaraları manuel olarak önceden kaydediyorsanız kapatın.',
    ...
    farmCooldownStallWindow: 'Soğuma takılma penceresi (dk)',
    farmCooldownStallWindowHelp: 'Çıkarma soğuması sırasında tabla soğumaya devam etmeli; ilerleme olmayan iki ardışık pencere yazıcıyı karantinaya alır (0 devre dışı bırakır)',
    farmCooldownStallEpsilon: 'Pencere başına min. soğuma (°C)',
    farmCooldownStallEpsilonHelp: 'İlerleme sayılan pencere başına minimum soğuma (°C)',
    farmCooldownMaxHold: 'Maks. soğuma bekleme (dk)',
    farmCooldownMaxHoldHelp: 'Bırakma eşiğinin üzerinde bu kadar dakika geçince çıkarma yine de çalışır (0 = eşiği süresiz bekle)',
    farmCooldownPlateauMargin: 'Plato çıkarma payı (°C)',
    farmCooldownPlateauMarginHelp: 'Soğuma, bırakma eşiğinin bu kadar °C içinde durağanlaşırsa, karantinaya almak yerine çıkar (tabla ortam sıcaklığına oturdu). Eşik + pay üzerinde tabla gerçekten sıcak takılıdır ve yazıcı karantinaya alınır',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: 'Mevcut kayıt #{{id}} — {{grams}} g kaldı — arşivlenecek',
