export default {
  ...
  productionRuns: {
    ...
    fields: {
      ...
      cooldownOverride: 'Soğutmayı geçersiz kıl (°C)',
      cooldownPlaceholder: 'Profil varsayılanını kullan',
      cooldownProfileDefault: 'Profil varsayılanını ({{value}}°C) geçersiz kılar',
  ...
  ejectProfiles: {
    ...
    fields: {
      name: 'Ad',
      cooldownTemp: 'Soğutma sıcaklığı (°C)',
      clearance: 'Boşluk (mm)',
      zOffset: 'Z ofseti (mm)',
    ...
    tooltips: {
      ...
      cooldownTemp: 'Süpürme başlamadan önce baskının soğuması gereken tabla sıcaklığı – parçalar soğuyunca tabladan ayrılır.',
    ...
    geometry: {
      ...
      cooldownAmbientTrap: 'Soğuma uyarı tabanının ({{floor}} °C) altında. Atölye ortam sıcaklığına eşit veya altındaki bir eşiğe asla ulaşılamaz ve çıkarma beklemesi sonsuza dek takılır — ortamın birkaç derece üzerine ayarlayın.',
    ...
    queueHelp: 'Tabla profil eşiğinin altına soğuduktan sonra parça süpürülür.',
  },
  // Navigasyon
  nav: {
    printers: 'Yazıcılar',
  ...
  settings: {
    ...
    farmCooldownWarnFloor: 'Soğuma uyarı tabanı (°C)',
    farmCooldownWarnFloorHelp: 'Bir çıkarma profilinin soğuma eşiği bunun altındaysa uyar — atölye ortamına eşit veya altındaki eşik asla tamamlanmaz (15–50)',
