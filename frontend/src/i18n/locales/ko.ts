export default {
  ...
  settings: {
    ...
    ejectUploadSkipIdentical: 'USB 드라이브에 이미 있는 배출 파일 재사용',
    ejectUploadSkipIdenticalHelp: '동일한 배출 파일이 이미 프린터의 USB 드라이브에 있으면 다시 업로드하지 않아, 배출마다 몇 초를 절약합니다.',
    ejectUploadSkipIdenticalWarning: '라이브 배출 프로브가 드라이브의 파일이 올바르게 재사용됨을 확인한 후에만 활성화하세요. 확신이 없으면 꺼 두세요. 다시 업로드하면 항상 올바른 파일이 보장됩니다.',
    ejectSlim3mf: '배출 파일 경량화',
    ejectSlim3mfHelp: '배출 파일에서 3D 모델과 미리보기 이미지를 제거해 더 빨리 업로드되게 합니다. 배출 동작 자체는 바뀌지 않습니다.',
    ejectSlim3mfWarning: '하드웨어 제한: 무인 생산에 활성화하기 전에 경량화된 파일로 전체 배출 하드웨어 래더를 완주하세요. 그 래더를 통과할 때까지 꺼 두세요.',
    powerStagger: '전력 스태거',
