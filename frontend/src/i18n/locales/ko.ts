export default {
  ...
  skus: {
    ...
    suggest: {
      title: '파일에서 제안',
      description: '슬라이스된 파일을 선택하면 해당 메타데이터에서 코드, 부품 번호, 이름을 미리 채웁니다.',
      file: '라이브러리 파일',
      filePlaceholder: '파일 선택…',
      action: '제안',
      applied: '{{source}}에서 정보를 채웠습니다',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: 'RFID 태그가 없는 AMS 트레이에 대해 재고 스풀을 자동으로 생성합니다. 중복을 피하기 위해 스풀을 수동으로 미리 등록하는 경우 끄십시오.',
    ...
    farmCooldownStallWindow: '냉각 정체 창 (분)',
    farmCooldownStallWindowHelp: '배출 냉각 중 베드는 계속 식어야 합니다. 진전 없는 창이 연속 2회면 프린터를 격리합니다 (0이면 비활성화)',
    farmCooldownStallEpsilon: '창당 최소 냉각 (°C)',
    farmCooldownStallEpsilonHelp: '진전으로 인정되는 창당 최소 냉각량 (°C)',
    farmCooldownMaxHold: '최대 냉각 대기 (분)',
    farmCooldownMaxHoldHelp: '해제 임계값을 초과한 채 이 시간이 지나면 배출을 강제로 실행합니다 (0 = 임계값까지 무기한 대기)',
    farmCooldownPlateauMargin: '정체 배출 여유값 (°C)',
    farmCooldownPlateauMarginHelp: '냉각이 배출 임계값 이내 이 °C에서 정체되면 격리 대신 배출합니다(베드가 주변 온도로 안정됨). 임계값 + 여유값을 초과하면 베드가 실제로 뜨겁게 멈춘 것이며 프린터가 격리됩니다',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: '현재 기록 #{{id}} — {{grams}} g 남음 — 보관 처리됩니다',
