export default {
  ...
  skus: {
    ...
    suggest: {
      title: '从文件建议',
      description: '选择一个已切片的文件，从其元数据预填编码、零件号和名称。',
      file: '库文件',
      filePlaceholder: '选择一个文件…',
      action: '建议',
      applied: '已从 {{source}} 填充信息',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: '为没有 RFID 标签的 AMS 料盘自动创建库存料卷。如果你手动预先登记料卷以避免重复，请关闭此项。',
    ...
    farmCooldownStallWindow: '冷却停滞窗口（分钟）',
    farmCooldownStallWindowHelp: '取件冷却期间热床必须持续降温；连续两个窗口没有进展则将打印机隔离（0 为禁用）',
    farmCooldownStallEpsilon: '每窗口最小降温（°C）',
    farmCooldownStallEpsilonHelp: '每个窗口算作进展的最小降温量（°C）',
    farmCooldownMaxHold: '最长冷却等待（分钟）',
    farmCooldownMaxHoldHelp: '在高于释放阈值超过这么多分钟后，仍会执行取件（0 = 无限期等待阈值）',
    farmCooldownPlateauMargin: '平台期顶出余量 (°C)',
    farmCooldownPlateauMarginHelp: '若冷却在释放阈值的该温度范围内停滞，则顶出（热床已降至环境温度）而非隔离打印机。超过阈值＋余量则热床确实卡在高温，打印机将被隔离',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: '当前记录 #{{id}} — 剩余 {{grams}} g — 将被归档',
