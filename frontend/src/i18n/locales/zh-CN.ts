export default {
  ...
  settings: {
    ...
    ejectUploadSkipIdentical: '复用 USB 驱动器上已有的退料文件',
    ejectUploadSkipIdenticalHelp: '当相同的退料文件已在打印机的 USB 驱动器上时，跳过重新上传，每次退料可节省几秒。',
    ejectUploadSkipIdenticalWarning: '仅在实时退料探测确认驱动器上的文件被正确复用后才启用。若不确定，请保持关闭——重新上传始终能保证使用正确的文件。',
    ejectSlim3mf: '精简退料文件',
    ejectSlim3mfHelp: '从退料文件中剥离 3D 模型和预览图，使其上传更快——退料动作本身保持不变。',
    ejectSlim3mfWarning: '受硬件限制：在为无人值守生产启用之前，请使用精简文件完整跑一遍退料硬件阶梯测试。在通过该阶梯测试之前请保持关闭。',
    powerStagger: '功率错峰',
