export default {
  ...
  skus: {
    ...
    suggest: {
      title: 'Suggest from file',
      description: 'Pick a sliced file to pre-fill the code, part number, and name from its metadata.',
      file: 'Library file',
      filePlaceholder: 'Select a file…',
      action: 'Suggest',
      applied: 'Details filled from {{source}}',
  ...
  settings: {
    ...
    autoAddUntaggedDesc: 'Silently create an inventory spool for AMS trays that have no RFID tag. Turn off if you pre-register spools manually to avoid duplicates.',
    ...
    farmCooldownStallWindow: 'Cooldown stall window (min)',
    farmCooldownStallWindowHelp: 'During eject cooldown the bed must keep cooling; two consecutive windows without progress quarantine the printer (0 disables)',
    farmCooldownStallEpsilon: 'Min cooling per window (°C)',
    farmCooldownStallEpsilonHelp: 'Minimum cooling (°C) per window that counts as progress',
    farmCooldownMaxHold: 'Max cooldown hold (min)',
    farmCooldownMaxHoldHelp: 'After this many minutes above the release threshold the eject runs anyway (0 = wait for threshold indefinitely)',
    farmCooldownPlateauMargin: 'Plateau eject margin (°C)',
    farmCooldownPlateauMarginHelp: 'If cooling plateaus within this many °C of the release threshold, eject (bed has settled at ambient) instead of quarantining. Above threshold + margin the bed is genuinely stuck hot and the printer is quarantined',
  ...
  inventory: {
    ...
    respool: {
      ...
      donorLine: 'Current record #{{id}} — {{grams}} g left — will be archived',
