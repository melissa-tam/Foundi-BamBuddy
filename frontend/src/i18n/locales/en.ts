export default {
  ...
  settings: {
    ...
    ejectUploadSkipIdentical: 'Reuse eject files already on the USB drive',
    ejectUploadSkipIdenticalHelp: 'Skip re-uploading an eject file when the identical one is already on the printer’s USB drive, saving a few seconds per eject.',
    ejectUploadSkipIdenticalWarning: 'Enable only after the live eject probe confirms the file on the drive is reused correctly. If in doubt, leave off — a re-upload always guarantees the right file.',
    ejectSlim3mf: 'Slim down eject files',
    ejectSlim3mfHelp: 'Strip the 3D model and preview image out of eject files so they upload faster — the eject motion itself is unchanged.',
    ejectSlim3mfWarning: 'Hardware-gated: run the full eject hardware ladder with slimmed files before enabling for unattended production. Leave off until that ladder passes.',
