...
async def test_stop_routes_through_the_one_operator_stop(db_session):
    """The stop + user-stopped-mark pair has ONE owner (``print_control``, 2026-09-12).

    This route keeps its own row transitions and its smart-plug step; what it must not
    keep is a second copy of the wire act, because a third copy is how the mark goes
    missing on the path that needs it — a cancel then reads as a print FAILURE.
    """
    ...
    with patch.object(
        print_control, "stop_as_operator", MagicMock(side_effect=lambda pid: stopped.append(pid) or True)
    ):
        await stop_queue_item(item.id, db=db_session, auth_result=(None, True))
