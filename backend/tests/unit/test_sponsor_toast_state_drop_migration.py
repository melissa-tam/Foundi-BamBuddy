"""Regression test for the ``sponsor_toast_state`` drop migration.

The in-app sponsor toast and its whole lane — check/dismiss routes, trigger service,
model, hook — were removed. The model is gone, so ``create_all`` no longer creates the
table on a fresh install; an EXISTING database still carries it, holding nothing but
which nag milestones had already fired. ``run_migrations`` drops it.
