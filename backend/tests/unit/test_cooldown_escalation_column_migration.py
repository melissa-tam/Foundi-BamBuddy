"""Regression test for the ``notification_providers.on_cooldown_escalation`` migration.

The cooldown-policy redesign introduced a dedicated ``cooldown_escalation`` event
(truthful "cooldown running long" copy, distinct from ``plate_not_empty``). Its
provider toggle is a boolean column added by ``run_migrations`` via ``_safe_execute``
(ADD COLUMN, default TRUE). ``create_all`` would create the column from the current
model and mask the migration, so the fixture drops it first to simulate a
pre-migration schema; the test then proves ``run_migrations`` re-adds it and that a
second pass is a no-op. Idempotent and SQLite-safe (mirrors the sibling migration
regression tests).
"""
...
_NEW_COLUMN = "on_cooldown_escalation"
