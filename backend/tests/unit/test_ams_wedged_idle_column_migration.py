"""Regression test for the ``notification_providers.on_ams_wedged_idle`` migration.

002-H2S (2026-09-11) added an ``ams_wedged_idle`` event: an AMS latched at
``ams_status_main == 1`` on an IDLE printer drops every load and unload, and the
scheduler's idle gate refuses to dispatch there — a hold with no print, no incident row
and no HMS code behind it, so nothing else in the farm can say why the printer stopped
taking work. Its provider toggle is a boolean column added by ``run_migrations`` via
``_safe_execute`` (ADD COLUMN, default TRUE), and its template is seeded from
``DEFAULT_TEMPLATES``.
...
_NEW_COLUMN = "on_ams_wedged_idle"
...
def test_template_is_registered_for_seeding():
    ...
    one ``notification_service.on_ams_wedged_idle`` actually supplies."""
    from backend.app.models.notification_template import DEFAULT_TEMPLATES

    row = next(t for t in DEFAULT_TEMPLATES if t["event_type"] == "ams_wedged_idle")
    assert row["name"] == "AMS Stuck Mid Filament-Change"
    supplied = {"printer_name", "minutes"}
