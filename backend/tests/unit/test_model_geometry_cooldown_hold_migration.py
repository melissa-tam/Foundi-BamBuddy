...
no default) and seeds the H2S row with the operator's 2026-09-10 PLACEHOLDERS —
keep-out Y 285 (the vendor's rear service area Y295 less 10 mm of margin) and clear
height 51 mm (eyewitness: clear to at least 50 mm above the nozzle plane with the
toolhead parked at the chute; the true maximum is still to be measured).
...
* re-running is a no-op and an operator/DB-set value survives (the ``IS NULL`` guard),
  matching the z_travel backfill's own contract;
...
async def test_migration_adds_columns_and_seeds_h2s(existing_engine):
    ...
    async with existing_engine.connect() as conn:
        ...
        # The 2026-09-10 placeholders: keep-out = vendor rear service Y295 - 10.
        assert await _hold(conn, "H2S") == (285.0, 51.0)
