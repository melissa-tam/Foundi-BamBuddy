...
from backend.app.services.archive import purge_paths
...
def owned_paths(row: LibraryFile) -> list[Path]:
    """The files on disk that vanish when *row* is hard-deleted.

    THE rule for "which bytes belong to this library row", so every lane that
    destroys one — the retention sweeper, the immediate hard delete, an external
    row dropped by :meth:`LibraryTrashService.trash_file`, and the user delete in
    ``services.user_deletion`` — removes the same set. The user delete needs it as
    a list rather than as an unlink call because it resolves what it will remove
    BEFORE its DELETEs run and only removes it once the transaction has committed;
    a rule that could only be expressed as "unlink now" would have forced that
    lane to hand-roll a second copy, which is how ``DELETE /users/{id}`` came to
    bypass this service altogether and orphan every byte it destroyed.

    An EXTERNAL row's bytes live outside Bambuddy's control — the thumbnail is the
    only file of its set that we created, so it is the only one we remove. That
    distinction previously lived in the CALLERS (``trash_file`` reached for a
    thumbnail-only variant while the sweeper reached for the both-files one),
    which is safe only for as long as every caller remembers which it is holding.
    """
    relative = (row.thumbnail_path,) if row.is_external else (row.file_path, row.thumbnail_path)
    resolved = (_to_absolute_path(rel) for rel in relative)
    return [path for path in resolved if path is not None]


def unlink_on_disk(row: LibraryFile) -> None:
    """Remove *row*'s files now — the "unlink" spelling of :func:`owned_paths`.

    For a caller that is about to drop ONE row and owns no useful rollback: the
    external-row branch of :meth:`LibraryTrashService.trash_file`, whose only
    casualty on a failed commit is a thumbnail the fork regenerates on demand.
    A lane deleting rows in bulk must resolve :func:`owned_paths` up front and
    call ``archive.purge_paths`` after its commit instead.
    """
    purge_paths(owned_paths(row))
...
class LibraryTrashService:
    ...
    async def _sweep(self, db: AsyncSession) -> int:
        ...
        # Resolve the bytes BEFORE the rows go (the path lives on the row) and
        # remove them only once the DELETE has committed: a commit that loses the
        # SQLite write lock must leave a restorable row, not a row whose file is
        # already gone.
        doomed = [path for row in rows for path in owned_paths(row)]
        ...
        await db.commit()
        purge_paths(doomed)
        logger.info("Library trash sweeper: hard-deleted %d row(s) past %d-day retention", len(rows), retention)
        return len(rows)
