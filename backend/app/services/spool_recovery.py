...
# tray_now sentinel: the EXTERNAL spool holder is feeding. Not an AMS slot, so the
# swap machine can neither unload it nor send an operator to a drawer for it
# (:func:`_valid_feeder` excludes it from every tray answer for the same reason).
_EXTERNAL_TRAY = 254
...
def _valid_feeder(value) -> int | None:
    ...
    :data:`_EXTERNAL_TRAY` (the external spool holder) and :data:`_NO_FILAMENT`
    (nothing fed) are sentinels, not trays: neither is something the swap machine can
    unload, nor a slot an operator can be sent to.
    """
    try:
        tray = int(value)
    except (TypeError, ValueError):
        return None
    return tray if 0 <= tray < _EXTERNAL_TRAY else None
